...
export class DashboardComponent implements OnInit {
  ...
  isExportando = signal<boolean>(false);

  exportarRelatorio() {
    this.isExportando.set(true);
    this.http.get('http://localhost:8080/api/relatorios/mercado/excel', { responseType: 'blob' }).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `RadarMercado_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      error: () => this.isExportando.set(false)
    });
  }
}
